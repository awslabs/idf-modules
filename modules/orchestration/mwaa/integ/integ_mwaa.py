import datetime
import logging
import os
import pathlib
import sys
import zipfile
from ast import literal_eval
from typing import Dict, List

import aws_cdk as cdk
import aws_cdk.cloud_assembly_schema as cas
import aws_cdk.integ_tests_alpha as integration
import boto3

sys.path.append("../")

from stack import MWAAStack  # noqa: E402

app = cdk.App()
timestamp = datetime.datetime.now()
logger = logging.getLogger(__name__)


directory = pathlib.Path("../plugins/")

with zipfile.ZipFile("plugins.zip", mode="w") as archive:
    for file_path in directory.rglob("*"):
        archive.write(file_path, arcname=file_path.relative_to(directory))


def get_module_dependencies(resource_keys: List[str]) -> Dict[str, str]:
    ssm = boto3.client("ssm", region_name="us-east-1")
    dependencies = {}
    try:
        for key in resource_keys:
            dependencies[key] = ssm.get_parameter(Name=f"/module-integration-tests/{key}")["Parameter"]["Value"]
    except Exception as e:
        print(f"issue getting dependencies: {e}")
    return dependencies


dependencies = get_module_dependencies(
    # Add any required resource identifiers here
    resource_keys=["vpc-id", "vpc-private-subnets", "s3-bucket"]
)

s3 = boto3.resource(service_name="s3", region_name="us-east-1")
s3.Object(dependencies["s3-bucket"], "plugins/plugins.zip").put(Body=open("plugins.zip", "rb"))

integration.IntegTest(
    app,
    "Integration Tests MWAA Module",
    test_cases=[
        MWAAStack(
            app,
            "integ-mwaa",
            project_name="integ",
            deployment_name="testing",
            module_name="mwaa",
            vpc_id=dependencies["vpc-id"],
            private_subnet_ids=literal_eval(dependencies["vpc-private-subnets"]),
            dag_path="dags",
            dag_bucket_name=dependencies["s3-bucket"],
            airflow_version="2.8.1",
            unique_requirements_file="../requirements/requirements.txt",
            stack_description=f"Integration Test {timestamp.hour}:{timestamp.minute}:{timestamp.second}",
            env=cdk.Environment(account=os.getenv("CDK_DEFAULT_ACCOUNT"), region="us-east-1"),
        )
    ],
    regions=["us-east-1"],
    enable_lookups=True,
    diff_assets=True,
    stack_update_workflow=True,
    cdk_command_options=cas.CdkCommands(
        deploy=cas.DeployCommand(args=cas.DeployOptions(require_approval=cas.RequireApproval.NEVER, json=True)),
        destroy=cas.DestroyCommand(args=cas.DestroyOptions(force=True)),
    ),
)
app.synth()
